...
class LogInApp(ctk.CTkFrame):
    def camuflage(self, x,y,w,h):
        ...
        crop = (x,y,x+w,y+h) 
        cropped_image = self.assets.rawBgGradient.crop(crop)
    ...
    def __init__(self, master, switch_callback):
        ...
        x,y=25,480
        w,h=112,30
        self.BtnForgotPassword = ctk.CTkLabel(self, text="Forgot password?", image=self.camuflage(x,y,w,h),font=self.assets.Crumbs,cursor="hand2")
        self.BtnForgotPassword.place(y=y, x=x)
        self.BtnForgotPassword.bind("<Button-1>", 1)

        x,y=205,679
        w,h=45,18
        self.BtnSignUp = ctk.CTkLabel(self, text="Sign up", width=w,height=h,image=self.camuflage(x,y,w,h),font=self.assets.Crumbs,cursor="hand2")
        ...
        self.BtnEnter.bind("<Button-2>", self.on_click)
